...
def get_cube_from_img(img3d, center, diameter_x,diameter_y,diameter_z):
    ...
    start_y = int(start_y)
    start_x = int(start_x)
    roi_img3d = img3d[start_z:start_z + diameter_z*2 , start_y:start_y + diameter_y, start_x:start_x + diameter_x *2]

    return roi_img3d

# based on the largest edge to cut the cube
def get_cube_from_img_base_large_edge(img3d, center, diameter_x,diameter_y,diameter_z):
    # get roi(z,y,z) image and in order the out of img3d(z,y,x)range
    center_z = center[0]
    center_y = center[1]
    center_x = center[2]
    ...
    if diameter_x >= diameter_z:
        diameter_xz = diameter_x
    else:
        diameter_xz = diameter_z

    start_x = max(center_x - diameter_xz, 0)
    if start_x + diameter_xz * 2 > img3d.shape[2]:
        start_x = img3d.shape[2] - diameter_xz * 2
    start_y = max(center_y - diameter_y/2, 0)
    if start_y + diameter_y > img3d.shape[1]:
        start_y = img3d.shape[1] - diameter_y
    start_z = max(center_z - diameter_xz , 0)
    if start_z + diameter_xz *2 > img3d.shape[0]:
        start_z = img3d.shape[0] - diameter_xz *2
    start_z = int(start_z)
    start_y = int(start_y)
    start_x = int(start_x)
    roi_img3d = img3d[start_z:start_z + diameter_xz*2, start_y:start_y + diameter_y, start_x:start_x + diameter_xz *2]
...
def get_node_classify():
    ...
    for subsetindex in range(1):
        ...
        output_path = "/home/huiying/luna/11.19/dataset/y/"
        ...
        for fcount, img_file in enumerate(tqdm(file_list)):
            ...
            if mini_df.shape[0] > 0:
                ...
                for node_idx, cur_row in mini_df.iterrows():
                    ...
                    node_cube = get_cube_from_img_base_large_edge(img_array, v_center, int(diameter_x),int(diameter_y),int(diameter_z))
                    node_cube.astype(np.uint8)
                    # # for y axis cut
                    save_size = node_cube.shape[1]
                    ...
                    for i in range(save_size):
                        ...
                        if save_size <= 11:
                            ...
                            if label == 1:
                                ...
                                # save for y axis
                                y_axis = node_cube[:, i, :]
                                cv2.imwrite(filepath + z + ".bmp", y_axis)
                            if label == 0:
